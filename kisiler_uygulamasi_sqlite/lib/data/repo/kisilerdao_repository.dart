...
class KisilerdaoRepository{
  ...
  Future<List<Kisiler>> kisileriYukle() async{
   var db = await VeritabaniYardimcisi.veritabaniErisim();//Veritabanını çağırdık
   List<Map<String,dynamic>> maps = await db.rawQuery("SELECT * FROM kisiler");//sorgu
  ...
  Future<List<Kisiler>> ara(String aramaKelimesi) async {
    var db = await VeritabaniYardimcisi.veritabaniErisim(); //Veritabanını çağırdık
    List<Map<String, dynamic>> maps = await db.rawQuery("SELECT * FROM kisiler WHERE kisi_ad like '%$aramaKelimesi%'"); //sorgu
